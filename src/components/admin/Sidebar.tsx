...
export default function Sidebar() {
  ...
  return (
    <aside className="w-64 h-screen bg-white border-r border-gray-200 flex flex-col fixed left-0 top-0 text-[#17204E] z-50">
      <div className="p-6 flex items-center gap-3 border-b border-gray-200">
        <div className="w-8 h-8 rounded-lg bg-[#057AF8] flex items-center justify-center">
      ...
      <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
        <div className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-4 mt-2 px-3">
          Overview
        </div>
            ...
            >
              {isActive && (
                <motion.div 
                  ...
                  className="absolute inset-0 bg-[#057AF8]/10 rounded-lg"
                  transition={{ type: "spring", stiffness: 300, damping: 30 }}
                />
              )}
              
              <div className="relative z-10 flex items-center gap-3">
                <Icon className={`w-5 h-5 ${isActive ? "text-[#057AF8]" : "text-gray-500 group-hover:text-[#17204E]"}`} />
                <span className={isActive ? "text-[#057AF8] font-bold" : "text-gray-600 group-hover:text-[#17204E]"}>
                  {item.name}
                </span>
              </div>
            </Link>
